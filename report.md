...
需要明确的是，本项目当前已经接入本地持久化向量库 Chroma。embedding 向量由远程接口在首次构建或资料变更时生成，并落盘保存到本地向量目录中，在后续服务重启时能够直接复用。因此，系统已经从“BM25 + in-memory embedding index”的阶段演进为“BM25 + persistent Chroma vector store”的混合检索实现。当前的主要工程边界不再是“是否有向量库”，而是该向量库仍然属于单机本地部署，而不是更大规模的服务化向量数据库。
...
- 向量由远程接口在索引构建阶段生成。
- 生成后的向量会写入本地持久化 Chroma 集合。
- 启动时会根据 chunk 签名复用已有向量，仅对新增或变更 chunk 重新写入。
...
需要明确指出，当前系统虽然已经实现 Hybrid Retrieval，并接入了本地持久化 Chroma 向量库，但它仍然不是“服务化大规模向量数据库”的最终形态。现阶段的真实状态是：

- 已实现：BM25 + persistent Chroma vector store
- 已支持：向量落盘、集合复用、重启后复用已有 embedding
- 未实现：更细粒度的增量向量更新策略与远程/分布式向量数据库部署

这意味着系统已经摆脱了“每次重启都必须全量重新向量化”的早期限制。首次建库时启动耗时仍然较高，但在后续重启场景中，系统可以直接复用已有持久化向量，只同步变更部分。
...
实际测试表明，首次全量构建时通常需要约 `75-80` 秒。如果继续使用同步构建模式，服务在索引完成前不会开始监听端口，用户会直接看到浏览器连接失败。当前版本在引入持久化 Chroma 后，第二次启动已可以复用现有向量集合，日志中可观察到 `复用 215 / 新写入 0` 的同步结果，启动压力明显降低。
...
- 当前使用的是本地单机 Chroma，尚未演进为更大规模的服务化向量数据库
- 首次建库和大规模资料更新时，启动时间仍然受语料规模和远程接口延迟影响明显
- 目前对“仅新增/修改个别 chunk”的增量更新还可以继续做得更细
- 自动化测试覆盖仍然不足
...
1. 将当前本地 Chroma 继续演进为更强的向量存储方案  
   例如接入 pgvector 或服务化向量数据库，提升可扩展性与运维能力。

2. 细化增量索引机制  
   仅为新增或变更文件中的受影响 chunk 重算向量，进一步减少构建耗时。
...
从课程项目视角看，这个系统不仅展示了 RAG 的基础思想，还体现了一个真实问答系统在多用户、持久化、文件管理、部署与运行优化等方面的工程复杂度。当前系统已经接入本地持久化 Chroma 向量库，并验证了“首次写入、后续复用”的运行链路，这使其相较早期原型具备了更完整的混合检索工程能力。
